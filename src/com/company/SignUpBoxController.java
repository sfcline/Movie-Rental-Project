...
public class SignUpBoxController {
  ...
  @FXML Button signUpBtn = new Button();
  @FXML RadioButton ageButton = new RadioButton();
  @FXML TextField username1 = new TextField();
  @FXML TextField username2 = new TextField();
  @FXML PasswordField password1 = new PasswordField();
  @FXML PasswordField password2 = new PasswordField();
  @FXML Label errorCodeTop = new Label();
  ...
  public boolean checkUsernameLength() {
    ...
    if ((username1.getText().trim()).length() < 6
        && (username2.getText().trim()).length() < 6) { // Both Fields < 6
      errorCodeTop.setTextFill(Paint.valueOf("#cc0000")); // Red
      errorCodeTop.setText("Both Username Fields Too Short.");
    } else if (((username1.getText().trim()).length() >= 6
        && (username2.getText().trim()).length() < 6)) { // Field 2 < 6
      errorCodeTop.setTextFill(Paint.valueOf("#cc0000")); // Red
      errorCodeTop.setText("Second Username Field Too Short.");
    } else if (((username1.getText().trim()).length() < 6
        && (username2.getText().trim()).length() >= 6)) { // Field 1 < 6
      errorCodeTop.setTextFill(Paint.valueOf("#cc0000")); // Red
  ...
  public boolean checkPasswordLength() {
    ...
    if ((password1.getText().trim()).length() < 6
        && (password2.getText().trim()).length() < 6) { // Both Fields < 6
      errorCodeBottom.setTextFill(Paint.valueOf("#cc0000")); // Red
      errorCodeBottom.setText("Both Password Fields Too Short.");
    } else if (((password1.getText().trim()).length() >= 6
        && (password2.getText().trim()).length() < 6)) { // Field 2 < 6
      errorCodeBottom.setTextFill(Paint.valueOf("#cc0000")); // Red
      errorCodeBottom.setText("Second Password Field Too Short.");
    } else if (((password1.getText().trim()).length() < 6
        && (password2.getText().trim()).length() >= 6)) { // Field 1 < 6
      errorCodeBottom.setTextFill(Paint.valueOf("#cc0000")); // Red
